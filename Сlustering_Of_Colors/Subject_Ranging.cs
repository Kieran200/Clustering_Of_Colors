...
namespace Сlustering_Of_Colors
{
    class Subject_Ranging
    {
        Random rnd = new Random();
         public void Ranging(int num_of_subject, int num_of_clusters)
        {
            //задаем выборку
            ...
            //вывод кластеров и их объектов
            for (int i = 0; i < 10; i++)
            {
                Console.WriteLine(i + ")  green: " + clusterList[i].center_green + "; red: " + clusterList[i].center_red + "; blue: " + clusterList[i].center_blue);
                foreach (Subject subject in clusterList[i].c_subjectList)
                    Console.WriteLine("green: " + subject.green_intensity + "; red: " + subject.red_intensity + "; blue: " + subject.blue_intensity);
            }
